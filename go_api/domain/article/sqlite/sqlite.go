...
func (sr *SqliteRepository) Create(a entity.Article) (entity.Article, error)  {
	tx, _ := sr.db.Begin()
	_, err := sr.db.Exec("INSERT INTO articles(article_id, title, content, published_at, author_id, topic_id) VALUES (?, ?, ?, ?, ?)", a.ArticleID, a.Title, a.Content, a.PublishedAt, a.ArticleID, a.TopicId)
